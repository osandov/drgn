...
drgn_program_register_module_debug_info_finder_impl(struct drgn_program *prog,
						    struct drgn_module_debug_info_finder *finder,
						    const char *name,
						    const struct drgn_module_debug_info_finder_ops *ops,
						    void *arg, size_t enable_index)
{
	...
	finder->ops = *ops;
	finder->arg = arg;
	err = drgn_handler_list_register(&prog->dbinfo.module_debug_info_finders,
...
LIBDRGN_PUBLIC struct drgn_error *
drgn_program_register_module_debug_info_finder(struct drgn_program *prog,
					       const char *name,
					       const struct drgn_module_debug_info_finder_ops *ops,
					       void *arg, size_t enable_index)
{
	return drgn_program_register_module_debug_info_finder_impl(prog, NULL,
								   name, ops,
								   arg,
								   enable_index);
}
...
LIBDRGN_PUBLIC struct drgn_error *
drgn_program_registered_module_debug_info_finders(struct drgn_program *prog,
						  const char ***names_ret,
						  size_t *count_ret)
{
	return drgn_handler_list_registered(&prog->dbinfo.module_debug_info_finders,
					    names_ret, count_ret);
}

LIBDRGN_PUBLIC struct drgn_error *
drgn_program_set_enabled_module_debug_info_finders(struct drgn_program *prog,
						   const char * const *names,
						   size_t count)
{
	return drgn_handler_list_set_enabled(&prog->dbinfo.module_debug_info_finders,
					     names, count,
					     "module debug info finder");
}

LIBDRGN_PUBLIC struct drgn_error *
drgn_program_enabled_module_debug_info_finders(struct drgn_program *prog,
					       const char ***names_ret,
					       size_t *count_ret)
{
	return drgn_handler_list_enabled(&prog->dbinfo.module_debug_info_finders,
					 names_ret, count_ret);
}
...
// DOCTODO: explain takes ownership of fd, path resolution
static struct drgn_error *
			     ...
			     size_t n, bool load_default, bool load_main)
{
	...
	// The module iterator may load debug info, so we need to check each
	// module again.
	...
	if (num_wanted_modules > 0) {
		uint64_t orig_supplementary_file_generation =
			prog->dbinfo.supplementary_file_generation;
		drgn_handler_list_for_each_enabled(struct drgn_module_debug_info_finder,
						   finder,
						   &prog->dbinfo.module_debug_info_finders) {
			  ...
			  struct drgn_program *prog)
{
	...
	drgn_program_register_symbol_finder_impl(prog, &dbinfo->symbol_finder,
						 ...
						 prog, 0);
	const struct drgn_module_debug_info_finder_ops
		standard_module_debug_info_finder_ops = {
			.find = drgn_standard_module_file_find,
		};
	drgn_program_register_module_debug_info_finder_impl(prog,
					&dbinfo->standard_module_debug_info_finder,
					"standard",
					&standard_module_debug_info_finder_ops,
					prog, 0);
	...
	if (drgn_have_debuginfod()) {
		const struct drgn_module_debug_info_finder_ops
			debuginfod_module_debug_info_finder_ops = {
				.find = drgn_debuginfod_find,
			};
		drgn_program_register_module_debug_info_finder_impl(prog,
					&dbinfo->debuginfod_module_debug_info_finder,
					"debuginfod",
					&debuginfod_module_debug_info_finder_ops,
					prog,
...
#endif
	drgn_handler_list_deinit(struct drgn_module_debug_info_finder, finder,
				 &dbinfo->module_debug_info_finders,
