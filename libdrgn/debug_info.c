...
static bool drgn_module_wants_file(struct drgn_module *module)
{
...
drgn_program_register_module_file_finder_impl(struct drgn_program *prog,
					      struct drgn_module_file_finder *finder,
					      const char *name,
					      const struct drgn_module_file_finder_ops *ops,
					      void *arg, size_t enable_index)
{
	...
	finder->ops = *ops;
	finder->arg = arg;
	err = drgn_handler_list_register(&prog->dbinfo.module_file_finders,
...
LIBDRGN_PUBLIC struct drgn_error *
drgn_program_register_module_file_finder(struct drgn_program *prog,
					 const char *name,
					 const struct drgn_module_file_finder_ops *ops,
					 void *arg, size_t enable_index)
{
	return drgn_program_register_module_file_finder_impl(prog, NULL, name,
							     ops, arg,
							     enable_index);
}

LIBDRGN_PUBLIC struct drgn_error *
drgn_program_registered_module_file_finders(struct drgn_program *prog,
					    const char ***names_ret,
					    size_t *count_ret)
{
	return drgn_handler_list_registered(&prog->dbinfo.module_file_finders,
					    names_ret, count_ret);
}

LIBDRGN_PUBLIC struct drgn_error *
drgn_program_set_enabled_module_file_finders(struct drgn_program *prog,
					     const char * const *names,
					     size_t count)
{
	return drgn_handler_list_set_enabled(&prog->dbinfo.module_file_finders,
					     names, count,
					     "module file finder");
}

LIBDRGN_PUBLIC struct drgn_error *
drgn_program_enabled_module_file_finders(struct drgn_program *prog,
					 const char ***names_ret,
					 size_t *count_ret)
{
	return drgn_handler_list_enabled(&prog->dbinfo.module_file_finders,
					 names_ret, count_ret);
}
...
#undef FORMAT
	...
	if (module->debug_file_status == DRGN_MODULE_FILE_WANT_SUPPLEMENTARY
	    || (check_build_id && module->build_id_len > 0)) {
	...
	} else if (check_build_id && module->build_id_len > 0) {
		if (elf_build_id_len < 0)
			return NULL;
		if (elf_build_id_len != module->build_id_len
		    || memcmp(elf_build_id, module->build_id,
			      elf_build_id_len) != 0) {
			if (elf_build_id_len == 0) {
				drgn_log_debug(prog,
					       "%s: %s is missing build ID",
					       module->name, path);
			} else {
				drgn_log_debug(prog,
					       "%s: %s build ID does not match",
					       module->name, path);
			}
			return NULL;
		}
		drgn_log_debug(prog, "%s: %s build ID matches", module->name,
			       path);
	}
	if (expected_crc) {
		size_t size;
...
drgn_module_try_standard_supplementary_files(struct drgn_module *module)
{
	...
	drgn_log_debug(prog,
		       "%s: trying standard paths for gnu_debugaltlink %s build ID %s in file %s",
		       module->name, debugaltlink_path,
...
static struct drgn_error *
load_debug_info_add_candidate(struct drgn_program *prog,
			      ...
			      const char *path)
{
	...
	if (!load_debug_info_file_vector_append(&it.entry->files, &file)) {
		if (load_debug_info_file_vector_empty(&it.entry->files)) {
			...
			load_debug_info_candidate_table_delete_iterator(&state->candidates,
									it);
		}
		return &drgn_enomem;
	}
...
static struct load_debug_info_candidate *
load_debug_info_find_candidate(struct load_debug_info_state *state,
...
static struct drgn_error *
load_debug_info_try_candidate(struct drgn_module *module,
			      struct load_debug_info_candidate *candidate,
			      enum drgn_module_file_status not_debug_file_status)
{
	struct drgn_error *err;
	vector_for_each(load_debug_info_file_vector, file, &candidate->files) {
		...
		// TODO: not 100% about these checks
		if (module->loaded_file_status != DRGN_MODULE_FILE_WANT
		    && module->debug_file_status != not_debug_file_status)
			break;
	}
	return NULL;
}
...
static struct drgn_error *
load_debug_info_try_supplementary_candidates(struct drgn_module *module,
					     struct load_debug_info_state *state)
{
	const void *checksum;
	size_t checksum_len;
	if (drgn_module_wanted_supplementary_debug_file(module, NULL, NULL,
		...
		return NULL;
	struct load_debug_info_candidate *candidate =
		load_debug_info_find_candidate(state, checksum, checksum_len);
	if (!candidate)
		return NULL;
	return load_debug_info_try_candidate(module, candidate,
					     DRGN_MODULE_FILE_WANT_SUPPLEMENTARY);
}

static struct drgn_error *
load_debug_info_try_candidates(struct drgn_module *module,
			       struct load_debug_info_state *state)
{
	// TODO: logging
	struct drgn_error *err;

	err = load_debug_info_try_supplementary_candidates(module, state);
	if (err)
		return err;
	...
	if (build_id_len != 0) {
		// Look up the candidate even if we don't need it so that it
		// counts as matched.
		struct load_debug_info_candidate *candidate =
			load_debug_info_find_candidate(state, build_id,
						       build_id_len);
		if (candidate && drgn_module_wants_file(module)) {
			uint64_t orig_supplementary_file_generation =
				module->prog->dbinfo.supplementary_file_generation;
			err = load_debug_info_try_candidate(module, candidate,
			...
			if (drgn_module_wanted_supplementary_debug_file_is_new(module,
						orig_supplementary_file_generation)) {
				err = load_debug_info_try_supplementary_candidates(module,
										   state);
				if (err)
					return err;
			}
			     ...
			     size_t n, bool load_default, bool load_main)
{
	...
	for (size_t i = 0; i < n; i++) {
		err = load_debug_info_add_candidate(prog, &state, paths[i]);
		if (err)
			return err;
	}
	...
	while (!(err = drgn_module_iterator_next(it, &module)) && module) {
		...
		err = load_debug_info_try_candidates(module, &state);
		if (err)
			return err;
	...
	if (num_wanted_modules > 0) {
		uint64_t orig_supplementary_file_generation =
			prog->dbinfo.supplementary_file_generation;
		drgn_handler_list_for_each_enabled(struct drgn_module_file_finder,
						   finder,
						   &prog->dbinfo.module_file_finders) {
			...
			for (size_t i = 0; i < num_wanted_modules; i++) {
				...
				// candidate for it one last time.
	...
	if (num_wanted_modules > 0) {
		for (size_t i = 0; i < num_wanted_modules; i++) {
			...
			if (drgn_module_loaded_file_status(module)
			    == DRGN_MODULE_FILE_WANT) {
...
LIBDRGN_PUBLIC struct drgn_error *
drgn_find_module_files(struct drgn_module **modules, size_t *num_modulesp)
{
	struct drgn_error *err;

	size_t num_modules = *num_modulesp;
	if (num_modules == 0)
		return NULL;

	struct drgn_program *prog = modules[0]->prog;
	size_t new_num_modules = 0;
	for (size_t i = 0; i < num_modules; i++) {
		...
		if (drgn_module_wants_file(modules[i]))
			modules[new_num_modules++] = modules[i];
	}
	...
	drgn_handler_list_for_each_enabled(struct drgn_module_file_finder,
					   finder,
					   &prog->dbinfo.module_file_finders) {
		err = finder->ops.find(modules, num_modules, finder->arg);
		if (err)
			return err;
		new_num_modules = 0;
		for (size_t i = 0; i < num_modules; i++) {
			  ...
			  struct drgn_program *prog)
{
	...
	drgn_program_register_symbol_finder_impl(prog, &dbinfo->symbol_finder,
						 ...
						 prog, 0);
	const struct drgn_module_file_finder_ops standard_module_file_finder_ops = {
		.find = drgn_standard_module_file_find,
	};
	drgn_program_register_module_file_finder_impl(prog,
						      &dbinfo->standard_module_file_finder,
						      "standard",
						      &standard_module_file_finder_ops,
						      prog, 0);
	dbinfo->debug_info_path = drgn_default_debug_info_path;
#if WITH_DEBUGINFOD
	dbinfo->debuginfod_client = NULL;
	if (drgn_have_debuginfod()) {
		const struct drgn_module_file_finder_ops debuginfod_module_file_finder_ops = {
			.find = drgn_debuginfod_find,
		};
		drgn_program_register_module_file_finder_impl(prog,
							      &dbinfo->debuginfod_module_file_finder,
							      "debuginfod",
							      &debuginfod_module_file_finder_ops,
							      prog,
...
#endif
	drgn_handler_list_deinit(struct drgn_module_file_finder, finder,
				 &dbinfo->module_file_finders,
