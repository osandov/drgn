		     ...
		     struct kallsyms_locations *loc)
{
	...
	for (size_t i = 0; i < kr->num_syms; i++) {
		...
		if (__builtin_add_overflow(names_idx, len + 1, &names_idx))
			return drgn_error_format(
				DRGN_ERROR_OTHER, "kallsyms: overflowed names_idx");
	}
 ...
 */
static struct drgn_error *
kallsyms_expand_symbol(struct kallsyms_reader *kr, uint32_t *offset,
		       struct string_builder *sb, char *kind_ret)
{
	if (*offset >= kr->names_len)
		return drgn_error_format(DRGN_ERROR_OTHER,
					 "kallsyms: invalid offset %" PRIu32 ", max offset is %zu",
					 *offset, kr->names_len);
	uint8_t *data = &kr->names[*offset];
	unsigned int len = *data;
	bool skipped_first = false;

	// Bounds checking: if we've overrun the buffer, we'll return an error
	// below when we bounds check *offset + len + 1
	if ((len & 0x80) && kr->long_names && *offset + 1 < kr->names_len) {
		data++;
		(*offset)++;
		len = (0x7F & len) | (*data << 7);
	}

	// Bounds check the bytes we will read.
	*offset += len;
	if (*offset >= kr->names_len)
		return drgn_error_format(DRGN_ERROR_OTHER,
					 "kallsyms: invalid offset %" PRIu32 ", max offset is %zu",
					 *offset, kr->names_len);
	// Leave offset pointing at the next token, which we will bounds check
	// on the next call.
	*offset += 1;
...
search_for_string(struct kallsyms_reader *kr, const char *name, ssize_t *ret)
{
	...
	for (ssize_t i = 0; i < kr->num_syms; i++) {
		...
		sb.len = 0;
		struct drgn_error *err =
			kallsyms_expand_symbol(kr, &names_idx, &sb, &kind);
		if (err)
			return err;
			  ...
			  struct drgn_symbol_index *ret)
{
	...
	for (int i = 0; i < kr.num_syms; i++) {
		...
		uint64_t size = 0;
		sb.len = 0;
		err = kallsyms_expand_symbol(&kr, &names_idx, &sb, &kind);
		if (err)
			return err;
		if (sb.len == 0)
			return drgn_error_format(DRGN_ERROR_OTHER,
