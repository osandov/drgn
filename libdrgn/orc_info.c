...
static int orc_version_from_header(const void *buffer)
{
	...
	static const uint8_t orc_hash_6_4[ORC_HASH_SIZE] = {
	...
	static const uint8_t orc_hash_6_3[ORC_HASH_SIZE] = {
		0xdb, 0x84, 0xae, 0xd4, 0x10, 0x3b, 0x31, 0xdd, 0x51, 0xe0,
		0x17, 0xf8, 0xf7, 0x97, 0x83, 0xca, 0x98, 0x5c, 0x2c, 0x51,
	};

	if (memcmp(buffer, orc_hash_6_4, ORC_HASH_SIZE) == 0)
		return 3;
	else if (memcmp(buffer, orc_hash_6_3, ORC_HASH_SIZE) == 0)
		return 2;
	return -1;
}
...
static struct drgn_error *drgn_read_orc_sections(struct drgn_module *module)
{
	...
	if (orc_header_scn) {
		...
		if (orc_header->d_size == ORC_HASH_SIZE)
...
{
	uint8_t header_data[ORC_HASH_SIZE];
...
#undef get_symbol
	...
	module->orc.version = -1;
	if (header_start && header_end && header_end - header_start != ORC_HASH_SIZE)
