...
enum drgn_module_kind {
	...
	DRGN_MODULE_VDSO,
	/** Relocatable file (e.g., Linux kernel loadable module). */
	DRGN_MODULE_RELOCATABLE,
	/**
	 * Extra debugging information that is not actually loaded in the
	 * program.
	 */
	DRGN_MODULE_EXTRA, // TODO: DRGN_MODULE_OTHER? DRGN_MODULE_MISC?
} __attribute__((__packed__));
...
struct drgn_module_key {
	...
	union {
		...
		struct {
			const char *name;
			/**
			 * Address that uniquely identifies the module (e.g.,
			 * for Linux kernel loadable modules, the base address).
			 */
			uint64_t address;
		} relocatable;
		struct {
...
// DOCTODO: make sure to call out that this is usually better for finders to use
// than _status
...
struct drgn_module_debug_info_finder_ops {
	void (*destroy)(void *arg);
...
// TODO: API for setting kernel directory for standard file finder?
