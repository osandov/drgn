...
// Always takes ownership of regs.
static struct drgn_error *
drgn_register_state_stack_trace(struct drgn_register_state *regs,
...
{
	struct drgn_error *err;
	_cleanup_thread_ struct drgn_thread *thread = NULL;
	err = drgn_program_find_thread(prog, tid, &thread);
	if (err)
		return err;
	if (!thread) {
		if (!(prog->flags & DRGN_PROGRAM_IS_LINUX_KERNEL)) {
			return drgn_error_create(DRGN_ERROR_LOOKUP, "thread not found");
		} else if (tid == 0) {
			return drgn_error_create(DRGN_ERROR_LOOKUP,
						 "task not found; "
	...
	return drgn_thread_stack_trace(thread, ret);
}
...
static struct drgn_error *
drgn_pt_regs_stack_trace(const struct drgn_object *obj,
			 struct drgn_stack_trace **ret)
{
	struct drgn_error *err;
	...
	struct drgn_register_state *regs;
	err = prog->platform.arch->pt_regs_get_initial_registers(obj, &regs);
	if (err)
		return err;
	return drgn_register_state_stack_trace(regs, ret);
}
			...
			struct drgn_stack_trace **ret)
{
	...
	if (drgn_type_kind(drgn_underlying_type(obj->type)) == DRGN_TYPE_INT) {
		...
		if (err)
			return err;
		return drgn_program_stack_trace(prog, value.uvalue, ret);
	}

	struct drgn_type *type = drgn_underlying_type(obj->type);
	const char *tag;
	if (drgn_type_kind(type) == DRGN_TYPE_STRUCT
	    && (tag = drgn_type_tag(type)) && strcmp(tag, "pt_regs") == 0) {
		DRGN_OBJECT(pt_regs_obj, prog);
		err = drgn_object_read(&pt_regs_obj, obj);
		if (err)
			return err;
		return drgn_pt_regs_stack_trace(&pt_regs_obj, ret);
	}
	...
	if ((prog->flags & DRGN_PROGRAM_IS_LINUX_KERNEL)
		...
		_cleanup_thread_ struct drgn_thread *thread = NULL;
		err = drgn_thread_from_object(obj, &thread);
		if (err)
			return err;
		return drgn_thread_stack_trace(thread, ret);
	} else if (strcmp(tag, "pt_regs") == 0) {
		DRGN_OBJECT(pt_regs_obj, prog);
		err = drgn_object_dereference(&pt_regs_obj, obj);
		if (err)
			return err;
		err = drgn_object_read(&pt_regs_obj, &pt_regs_obj);
		if (err)
			return err;
		return drgn_pt_regs_stack_trace(&pt_regs_obj, ret);
	}
...
{
	struct drgn_register_state *regs;
	struct drgn_error *err = drgn_thread_register_state(thread, &regs);
	if (err)
		return err;
	if (!regs) {
		...
		if (drgn_program_is_userspace_process(prog)
		    && !drgn_handler_list_has_registered(&prog->register_state_finders)) {
