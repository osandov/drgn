...
py_module_debug_info_find_fn(struct drgn_module * const *modules,
...
#define module_debug_info_finder_arg(self, fn) PyObject *arg = fn;
...
DEFINE_PROGRAM_FINDER_METHODS(module_debug_info)
...
{
	struct drgn_error *err;
	static char *keywords[] = {"name", "address", NULL};
	struct path_arg name = {};
	struct index_arg address = {};
	if (!PyArg_ParseTupleAndKeywords(args, kwds, "O&O&:relocatable_module",
					 keywords, path_converter, &name,
					 index_converter, &address))
		return NULL;

	struct drgn_module *module;
	err = drgn_module_find_or_create_relocatable(&self->prog, name.path,
						     address.uvalue, &module,
						     NULL);
	path_cleanup(&name);
	if (err) {
		set_drgn_error(err);
...
static PyObject *Program_find_relocatable_module(Program *self, PyObject *args,
						 PyObject *kwds)
{
	static char *keywords[] = {"name", "address", NULL};
	struct path_arg name = {};
						...
						PyObject *kwds)
{
	...
	if (!PyArg_ParseTupleAndKeywords(args, kwds, "O:load_module_debug_info",
	...
	struct drgn_error *err =
		drgn_load_module_debug_info(drgn_module_vector_begin(&modules),
					    &num_modules);
	if (err)
		return set_drgn_error(err);
	Py_RETURN_NONE;
...
static PyMethodDef Program_methods[] = {
	...
	PROGRAM_FINDER_METHOD_DEFS(module_debug_info),
	...
	{"find_relocatable_module",
	 (PyCFunction)Program_find_relocatable_module,
	 METH_VARARGS | METH_KEYWORDS,
	 drgn_Program_find_relocatable_module_DOC},
	...
	{"load_module_debug_info", (PyCFunction)Program_load_module_debug_info,
	 METH_VARARGS | METH_KEYWORDS, drgn_Program_load_module_debug_info_DOC},
