...
{
	_cleanup_pydecref_ PyObject *py_message = PyObject_GetAttrString(exc_value, "message");
	const char *message = py_message ? PyUnicode_AsUTF8(py_message) : NULL;
	if (!message)
		return NULL;

	_cleanup_pydecref_ PyObject *py_address = PyObject_GetAttrString(exc_value, "address");
	uint64_t address;
	if (py_address)
		address = PyLong_AsUint64(py_address);
	if (!py_address || PyErr_Occurred())
		return NULL;
