...
// DOCTODO
struct drgn_module_debug_info_finder {
	struct drgn_handler handler;
	struct drgn_module_debug_info_finder_ops ops;
	void *arg;
};
...
struct drgn_debug_info {
	...
	struct drgn_module *main_module; // DOCTODO
	struct drgn_module_table modules; // DOCTODO
	/**
	...
	struct drgn_module_address_tree modules_by_address; // DOCTODO
	struct drgn_module *modules_pending_indexing;
	/** DWARF debugging information. */
	struct drgn_dwarf_info dwarf;

	struct drgn_handler_list module_debug_info_finders;
	struct drgn_module_debug_info_finder standard_module_debug_info_finder;
	const char *debug_info_path;
	// DOCTODO: explain. Specifically, this is the last generation number
	// given out, not the next to give out.
	uint64_t supplementary_file_generation;

#if WITH_DEBUGINFOD
	struct drgn_module_debug_info_finder debuginfod_module_debug_info_finder;
...
struct drgn_module {
	...
	// DOCTODO: better wording for this documentation?
