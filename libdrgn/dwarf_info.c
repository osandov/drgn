...
#undef X
	...
	for (;;) {
		...
		if (!drgn_type_has_tag(qualified_type.type)
							...
							union drgn_lazy_object *argument)
{
	...
	if (!string_builder_append(&name_builder->sb, formatted))
		return &drgn_enomem;
	return NULL;
}
	...
	for (size_t i = num_scopes; i-- > 1;) {
		...
		case DW_TAG_inlined_subroutine:
			...
			*name_ret = dwarf_diename(&scopes[i]);
			...
			struct drgn_elf_file *scope_file =
				drgn_module_find_dwarf_file(module, dw);
