...
static int addr_compar(const void *lhs, const void *rhs, void *args)
{
		       ...
		       char *buffer, struct drgn_symbol_index *ret)
{
	...
	for (uint32_t i = 1; i < ret->num_syms; i++) {
		if (ret->symbols[i - 1].address > ret->symbols[i].address) {
			qsort_r(ret->symbols, count, sizeof(ret->symbols[0]), addr_compar, NULL);
			break;
		}
	}
	...
	for (uint32_t i = 0; i < ret->num_syms; i++)
		ret->name_sort[i] = i;
	qsort_r(ret->name_sort, ret->num_syms, sizeof(ret->name_sort[0]),
		name_compar, ret);
	...
	while (current < ret->num_syms) {
		...
		if (drgn_symbol_name_table_insert(&ret->htab, &entry, NULL) < 0) {
			free(ret->name_sort);
			free(ret->max_addrs);
			drgn_symbol_name_table_deinit(&ret->htab);
			ret->name_sort = NULL;
			return &drgn_enomem;
		}
				    ...
				    struct drgn_symbol_index_builder *builder)
{
	...
	struct drgn_error *err =
		drgn_symbol_index_init(symbols, num_syms, names, index);
	if (err) {
		free(names);
		free(symbols);
	}
