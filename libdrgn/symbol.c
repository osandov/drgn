...
{
	if (builder->one) {
		// As an optimization, reuse the existing symbol allocation if
		// we can.
		if (!builder->single
		    || builder->single->lifetime == DRGN_LIFETIME_STATIC) {
			builder->single = malloc(sizeof(*builder->single));
			if (!builder->single)
				return false;
		} else if (builder->single->name_lifetime == DRGN_LIFETIME_OWNED) {
			free((char *)builder->single->name);
		}
