...
class TestModuleTryFile(TestCase):
    ...
    def test_want_both(self):
        module = self.prog.extra_module("/foo/bar", 0, create=True)[0]
    ...
    def test_gnu_debugaltlink(self):
        ...
        with tempfile.TemporaryDirectory(
            ...
            module = self.prog.extra_module(bin_dir / "binary", 0, create=True)[0]
...
class TestLinuxUserspaceCoreDump(TestCase):
    ...
    def test_loaded_modules(self):
        ...
        with self.subTest(module="libc"):
            module = self.prog.shared_library_module("/lib64/libc.so.6", 0x7FE154D0BB80)
            found_modules.append(module)
        ...
        with self.subTest(module="vdso"):
            module = self.prog.vdso_module("linux-vdso.so.1", 0x7FFEB18EC3E0)
            found_modules.append(module)
    ...
    def test_loaded_modules_pie(self):
        ...
        with self.subTest(module="libc"):
            module = self.prog.shared_library_module("/lib64/libc.so.6", 0x7FB63CE22B80)
            found_modules.append(module)
        ...
        with self.subTest(module="vdso"):
            module = self.prog.vdso_module("linux-vdso.so.1", 0x7FFF5557C3E0)
            found_modules.append(module)
    ...
    def test_loaded_modules_static(self):
        ...
        with self.subTest(module="vdso"):
            module = self.prog.vdso_module("linux-vdso.so.1", 0x7FFC12B533E0)
            found_modules.append(module)
    ...
    def test_loaded_modules_static_pie(self):
        ...
        with self.subTest(module="vdso"):
            module = self.prog.vdso_module("linux-vdso.so.1", 0x7FFD67DF13E0)
            found_modules.append(module)
    ...
    def test_loaded_modules_pie_no_headers(self):
        ...
        with self.subTest(module="vdso"):
            module = self.prog.vdso_module("linux-vdso.so.1", 0x7FFCE47BB3E0)
            found_modules.append(module)
            self.assertEqual(module.address_range, (0x7FFCE47BB000, 0x7FFCE47BBD5D))
            self.assertEqual(
                module.build_id.hex(), "320b9b38597b3c1894dc1a40674729b29a2de12c"
            )
        ...
        # If we can read the program header table from AT_PHDR, then we should
        # be able to get most of the modules (with limited information).
        exe_file = self.enterContext(open(get_resource("crashme_pie"), "rb"))

        def read_at_phdr(address, count, offset, physical):
            exe_file.seek(0x40 + offset)
            return exe_file.read(count)

        self.prog.add_memory_segment(0x55C080D67040, 728, read_at_phdr, False)
        ...
        with self.subTest(module="crashme"):
            module = self.prog.shared_library_module(
                "/home/osandov/crashme.so", 0x7F424B705E20
            )
        ...
        with self.subTest(module="libc"):
            module = self.prog.shared_library_module("/lib64/libc.so.6", 0x7F424B6E4B80)
            new_found_modules.append(module)
            self.assertIsNone(module.address_range)
            self.assertIsNone(module.build_id)

        # ld-linux.so is missing because its name is not saved in the core dump.
    ...
    def test_default_by_finder(self):
        ...
        self.assertCountEqual(
            ...
            [
                ...
                self.prog.vdso_module("linux-vdso.so.1", 0x7FFCE47BB3E0),
                self.prog.shared_library_module(
                    "/home/osandov/crashme.so", 0x7F424B705E20
                ),
                self.prog.shared_library_module("/lib64/libc.so.6", 0x7F424B6E4B80),
            ],
        )
        # The finder should be called and set the files for the main and
        # crashme.so modules.
        self.assertEqual(
            self.prog.main_module().loaded_file_path,
            str(crashme_pie_path),
...
class TestLoadModuleDebugInfo(TestCase):
    ...
    def test_multiple(self):
        self.prog.load_module_debug_info(
            self.prog.extra_module("/foo/bar", 0, create=True)[0],
            self.prog.extra_module("/foo/baz", 0, create=True)[0],
        )
        self.finder.assert_called_once()
        self.assertCountEqual(
            self.finder.call_args.args[0],
            [
                self.prog.extra_module("/foo/bar", 0),
                self.prog.extra_module("/foo/baz", 0),
            ],
        )
    ...
    def test_wrong_program(self):
        self.assertRaisesRegex(
            ...
            self.prog.load_module_debug_info,
            self.prog.extra_module("/foo/bar", 0, create=True)[0],
            Program().extra_module("/foo/baz", 0, create=True)[0],
        )
...
class TestStandardDebugInfoFinder(TestCase):
    ...
    def test_by_module_name(self):
        with NamedTemporaryElfFile() as f:
            module = self.prog.extra_module(f.name, 0, create=True)[0]
            self.prog.load_module_debug_info(module)
        self.assertEqual(module.loaded_file_status, ModuleFileStatus.HAVE)
        self.assertEqual(module.debug_file_status, ModuleFileStatus.HAVE)
        self.assertEqual(module.loaded_file_path, f.name)
...
class TestDebuginfodDebugInfoFinder(TestCase):
    ...
    def test_no_build_id(self):
        module = self.prog.extra_module("foo", 0, create=True)[0]
        self.prog.load_module_debug_info(module)
        self.assertEqual(module.loaded_file_status, ModuleFileStatus.WANT)
        self.assertEqual(module.debug_file_status, ModuleFileStatus.WANT)
