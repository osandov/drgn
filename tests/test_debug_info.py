...
class TestModuleTryFile(TestCase):
    def setUp(self):
        self.prog = Program()
        self.prog.set_enabled_module_debug_info_finders([])
...
class TestLoadDebugInfo(TestCase):
    def setUp(self):
        ...
        self.prog.register_module_debug_info_finder("mock", self.finder, enable_index=0)
...
class TestLoadDebugInfoCoreNoHeaders(TestCase):
    ...
    # TODO: we need load_debug_info() to match by something other than build ID
    # for this case.
    @unittest.expectedFailure
    def test_main_by_path(self):
        crashme_pie_path = get_resource("crashme_pie")

        self.prog.load_debug_info([crashme_pie_path], main=True)
    ...
    # for this case.
    @unittest.expectedFailure
    def test_default_by_path(self):
        crashme_pie_path = get_resource("crashme_pie")
        crashme_so_path = get_resource("crashme.so")

        try:
            self.prog.load_debug_info([crashme_pie_path, crashme_so_path], default=True)
        ...
        self.assertCountEqual(
            list(self.prog.created_modules()),
            [
                self.prog.find_main_module(),
                self.prog.find_vdso_module("linux-vdso.so.1", 0x7FFCE47BB3E0),
                self.prog.find_shared_library_module(
        ...
        # The provided paths should be used for the main and crashme.so
        # modules.
        self.assertEqual(
            self.prog.find_main_module().loaded_file_path,
            str(crashme_pie_path),
...
class TestDebuginfodModuleFileFinder(TestCase):
    ...
    def setUp(self):
        self.prog = Program()
        try:
            self.prog.set_enabled_module_debug_info_finders(["debuginfod"])
        except ValueError:
