...
class TestSearchMemoryBytes(TestCase):
    def test_one_match(self):
        prog = mock_program(segments=[MockMemorySegment(b"abcdef", 0x1000)])
        self.assertEqual(list(prog.search_memory(b"cde")), [0x1002])

    def test_haystack_embedded_null(self):
        prog = mock_program(segments=[MockMemorySegment(b"a\0bcdef\0g", 0x1000)])
        self.assertEqual(list(prog.search_memory(b"cde")), [0x1003])

    def test_needle_embedded_null(self):
        prog = mock_program(segments=[MockMemorySegment(b"ab\0c\0d", 0x1000)])
        self.assertEqual(list(prog.search_memory(b"b\0c")), [0x1001])

    def test_multiple_matches(self):
        prog = mock_program(segments=[MockMemorySegment(b"abcd abcz", 0x1000)])
        self.assertEqual(list(prog.search_memory(b"abc")), [0x1000, 0x1005])

    def test_non_overlapping(self):
        prog = mock_program(segments=[MockMemorySegment(b"aaaa", 0x1000)])
    ...
    def test_invalid_address_range(self):
        self.assertRaises(
            ValueError,
            mock_program().search_memory(b"foo").set_address_range,
            0x1000,
            0xFFF,
        )

    def test_cross_boundary(self):
        prog = mock_program(segments=[MockMemorySegment(b"foo bar", 0x3FFFFFFA)])
    ...
    def test_alignment_at_max_address(self):
        prog = mock_program(segments=[MockMemorySegment(b"a" * 12, 0xFFFFFFFFFFFFFFF4)])
        self.assertEqual(
            list(prog.search_memory(b"aa", alignment=8)),
            [0xFFFFFFFFFFFFFFF8],
        )
    ...
    def test_empty(self):
        with self.assertRaisesRegex(ValueError, "needle cannot be empty"):
            next(mock_program().search_memory(b""))

    def test_invalid_alignment(self):
        with self.assertRaisesRegex(ValueError, "alignment must be power of 2"):
            next(mock_program().search_memory(b"foo", alignment=3))

    def test_invalid_needle_type(self):
        self.assertRaises(TypeError, mock_program().search_memory, {})

    def test_default_program(self):
        with with_default_prog(
            mock_program(segments=[MockMemorySegment(b"abcdef", 0x1000)])
        ):
...
class TestSearchMemoryStr(TestCase):
    ...
    def test_invalid_utf8(self):
        prog = mock_program(
            segments=[MockMemorySegment(b"\xc3\x28abcdef\xa0\xa1", 0x1000)]
        )
        self.assertEqual(list(prog.search_memory("cde")), [0x1004])

    def test_empty(self):
        with self.assertRaisesRegex(ValueError, "empty"):
            next(mock_program().search_memory(""))
    ...
    def test_invalid_alignment(self):
        with self.assertRaisesRegex(ValueError, "alignment"):
            next(mock_program().search_memory("foo", alignment=3))
...
class TestSearchMemoryInt(TestCase):
    ...
    def test_invalid_alignment(self):
        with self.assertRaisesRegex(TypeError, "alignment"):
            next(mock_program().search_memory(0, alignment=3))
...
class TestSearchMemoryObject(TestCase):
    ...
    def test_small_object(self):
        prog = mock_program(
            Platform(Architecture.X86_64),
            segments=[MockMemorySegment(b"\x01\x02\x03\x04", 0x1000)],
        )
    ...
    def test_alignment(self):
        prog = mock_program(
            Platform(Architecture.X86_64),
            segments=[MockMemorySegment(b"\x02\x00\x00\x00" * 7, 0xFFC)],
        )
        self.assertEqual(
    ...
    def test_empty(self):
        prog = mock_program(Platform(Architecture.X86_64))
    ...
    def test_out_of_range(self):
        with self.assertRaises(OverflowError):
            next(mock_program(MOCK_32BIT_PLATFORM).search_memory(2**32))
        self.assertEqual(list(mock_program().search_memory(2**32)), [])
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory(2**64))
...
class TestSearchMemoryUint(TestCase):
    ...
    def test_out_of_range(self):
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_u32(2**32))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_u32((0, 2**32)))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_u32((2**32, 0)))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_u32(0, ignore_mask=2**32))
        with self.assertRaises((ValueError, OverflowError)):
            next(mock_program().search_memory_u32(-1))

        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_u64(2**64))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_u64((0, 2**64)))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_u64((2**64, 0)))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_u64(0, ignore_mask=2**64))
        with self.assertRaises((ValueError, OverflowError)):
            next(mock_program().search_memory_u64(-1))

        with self.assertRaises(OverflowError):
            next(mock_program(MOCK_32BIT_PLATFORM).search_memory_word(2**32))
        with self.assertRaises(OverflowError):
            next(mock_program(MOCK_32BIT_PLATFORM).search_memory_word((0, 2**32)))
        with self.assertRaises(OverflowError):
            next(mock_program(MOCK_32BIT_PLATFORM).search_memory_word((2**32, 0)))
        with self.assertRaises(OverflowError):
            next(
                mock_program(MOCK_32BIT_PLATFORM).search_memory_word(
                    0, ignore_mask=2**32
                )
            )
        ...
        self.assertEqual(
            list(
                mock_program().search_memory_word(
        ...
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_word(2**64))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_word((0, 2**64)))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_word((2**64, 0)))
        with self.assertRaises(OverflowError):
            next(mock_program().search_memory_word(0, ignore_mask=2**64))


@unittest.skipUnless(drgn._with_pcre2, "built without pcre2 support")
class TestSearchMemoryRegex(TestCase):
    def test_fixed_size(self):
        prog = mock_program(segments=[MockMemorySegment(b"foo bar", 0x1000)])
        self.assertEqual(
    ...
    def test_non_overlapping(self):
        prog = mock_program(segments=[MockMemorySegment(b"abcd", 0x1000)])
        self.assertEqual(
    ...
    def test_variable_size(self):
        prog = mock_program(segments=[MockMemorySegment(b"caaaaat", 0x1000)])
        self.assertEqual(
    ...
    def test_gap(self):
        prog = mock_program(
            segments=[
                MockMemorySegment(b"caaaaa", 0x1000),
                MockMemorySegment(b"aaaaat", 0x1008),
            ]
        )
        self.assertEqual(
            list(prog.search_memory_regex(rb"a+")),
    ...
    def test_all_empty(self):
        prog = mock_program(segments=[MockMemorySegment(b"b", 0x1000)])
        self.assertEqual(
            list(prog.search_memory_regex(rb"a*")),
            [
                (0x1000, b""),
    ...
    def test_empty_at_gap(self):
        prog = mock_program(
            segments=[MockMemorySegment(b"b", 0x1000), MockMemorySegment(b"c", 0x1008)]
        )
        self.assertEqual(
            list(prog.search_memory_regex(rb"a*")),
            [
    ...
    def test_anchor_start(self):
        prog = mock_program(
            segments=[MockMemorySegment(b"abc", 0x0), MockMemorySegment(b"abc", 0x1000)]
        )
        self.assertEqual(
    ...
    def test_cross_boundary_fixed_size(self):
        prog = mock_program(segments=[MockMemorySegment(b"foo bar", 0x3FFFFFFE)])
        self.assertEqual(
            list(prog.search_memory_regex(rb"foo|bar")),
            [
                (0x3FFFFFFE, b"foo"),
    ...
    def test_buffer_capacity_at_boundary(self):
        prog = mock_program(segments=[MockMemorySegment(b"ab", 0x7FFFFFFF)])
        self.assertEqual(
    ...
    def test_str_valid_utf8(self):
        ...
        prog = mock_program(segments=[MockMemorySegment("piñata±".encode(), 0x1000)])
        self.assertEqual(
    ...
    def test_no_lookbehind(self):
        self.assertRaisesRegex(
            ValueError,
            "lookbehind",
            mock_program().search_memory_regex,
            rb"(?<=foo)bar",
        )
        self.assertRaisesRegex(
            ValueError,
    ...
    def test_default_program(self):
        with with_default_prog(
            mock_program(segments=[MockMemorySegment(b"foo bar", 0x1000)])
        ):
            self.assertEqual(
