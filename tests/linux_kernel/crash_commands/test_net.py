...
class TestNet(CrashCommandTestCase):
    def test_net(self):
        cmd = self.check_crash_command("net")

        self.assertFalse(cmd.stderr)
        self.assertRegex(cmd.stdout, r"(?m)^\s*NET_DEVICE\s+NAME\s+IP ADDRESS\(ES\)$")
        self.assertIn("for_each_netdev()", cmd.drgn_option.stdout)
        for name in ("dev", "name", "ipv4_addrs", "ipv6_addrs"):
    ...
    def test_net_arp(self):
        ...
        cmd = self.check_crash_command("net -a")

        self.assertFalse(cmd.stderr)
        self.assertRegex(
            cmd.stdout,
            r"(?m)^\s*NEIGHBOUR\s+IP ADDRESS\s+HW TYPE\s+HW ADDRESS\s+DEVICE\s+STATE$",
        )
        self.assertIn("neigh_table_for_each_neighbor(", cmd.drgn_option.stdout)

        neigh = neighbors[0]
        ip_addr = str(
            ipaddress.IPv4Address(self.prog.read(neigh.primary_key.address_of_(), 4))
        )
        hw_addr = self.prog.read(neigh.ha.address_of_(), int(neigh.dev.addr_len)).hex(
            sep=":"
        )
