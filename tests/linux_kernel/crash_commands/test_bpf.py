...
class TestBpf(CrashCommandTestCase, BpfTestCase):
    ...
    def test_bpf_map_by_id(self):
        with contextlib.ExitStack() as exit_stack:
            ...
            self.assertRegex(
                cmd.stdout,
                r"(?sm)^\s*KEY_SIZE:\s*4\s+VALUE_SIZE:\s*8\s+MAX_ENTRIES:\s*256\s+MEMLOCK:\s*\d+",
            )

            self.assertRegex(
                cmd.stdout,
                r"(?sm)^\s*NAME:\s*(\(unused\)|\(unknown\)|\"[^\"]*\")\s+UID:\s*(\d+|\(unknown\)|\(unused\))",
            )
    ...
    def test_bpf_map_by_id_with_program(self):
        with contextlib.ExitStack() as exit_stack:
            ...
            self.assertIn("MEMLOCK:", cmd.stdout)
