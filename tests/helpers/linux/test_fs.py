...
class TestFs(LinuxHelperTestCase):
    def test_path_lookup(self):
        with tempfile.NamedTemporaryFile(prefix='drgn-tests-') as f:
            ...
            self.assertEqual(path.dentry.d_name.name.string_(),
                             os.fsencode(os.path.basename(f.name)))
    ...
    def test_d_path(self):
        ...
        self.assertEqual(d_path(task.fs.pwd.address_of_()),
                         os.fsencode(os.getcwd()))
        ...
        with tempfile.TemporaryDirectory(prefix='drgn-tests-') as dir:
            ...
            with open(path1, 'w') as f:
                ...
                with open(path2, 'r') as f:
                    ...
                    self.assertTrue((path1.endswith(paths[0]) and
                                     path2.endswith(paths[1])) or
                                    (path1.endswith(paths[1]) and
                                     path2.endswith(paths[0])))
                    self.assertIn(inode_path(inode), paths)

    def test_for_each_mount(self):
        with open('/proc/self/mounts', 'rb') as f:
            self.assertEqual({line.split()[1].decode('unicode-escape')
                              for line in f},
                             {os.fsdecode(mount_dst(mount))
                              for mount in for_each_mount(self.prog)})

    def test_fget(self):
        with tempfile.NamedTemporaryFile(prefix='drgn-tests-') as f:
            file = fget(find_task(self.prog, os.getpid()), f.fileno())
            self.assertEqual(d_path(file.f_path),
                             os.fsencode(os.path.abspath(f.name)))
    ...
    def test_for_each_file(self):
        ...
        with os.scandir('/proc/self/fd') as dir:
            ...
            self.assertEqual({fd for fd, file in for_each_file(task)},
