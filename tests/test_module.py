...
class TestModule(TestCase):
    ...
    def test_address_range(self):
        module = Program().extra_module("/foo/bar", 0, create=True)[0]
    ...
    def test_find_by_address(self):
        prog = Program()
        module1 = prog.extra_module("/foo/bar", 0, create=True)[0]
        module1.address_range = (0x10000000, 0x10010000)
        module2 = prog.extra_module("/asdf/jkl", 0, create=True)[0]
...
class TestCreatedModules(TestCase):
    ...
    def test_multiple(self):
        prog = Program()
        modules = [
            prog.extra_module("/foo/bar", 0, create=True)[0],
            prog.extra_module("/asdf/jkl", 0, create=True)[0],
            prog.extra_module("/123/456", 0, create=True)[0],
        ]
        self.assertCountEqual(list(prog.modules()), modules)

    def test_change_during_iteration(self):
        prog = Program()
        prog.extra_module("/foo/bar", 0, create=True)
        with self.assertRaisesRegex(Exception, "modules changed during iteration"):
            for module in prog.modules():
                prog.extra_module("/asdf/jkl", 0, create=True)
                prog.extra_module("/123/456", 0, create=True)
