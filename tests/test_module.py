...
class TestModule(TestCase):
    ...
    def test_main_module(self):
        ...
        self.assertEqual(prog.main_module("/baz"), module)  # DOCTODO: document this
    ...
    def test_relocatable_module(self):
        ...
        self.assertRaises(
            LookupError, prog.find_relocatable_module, "/foo/bar", 0x10000000
        )

        module = prog.relocatable_module("/foo/bar", 0x10000000)
        self.assertIsInstance(module, RelocatableModule)

        self.assertEqual(prog.find_relocatable_module("/foo/bar", 0x10000000), module)
        self.assertRaises(
            LookupError, prog.find_relocatable_module, "/foo/bar", 0x20000000
        )
        ...
        self.assertNotEqual(prog.relocatable_module("/foo/bar", 0x20000000), module)
        self.assertNotEqual(prog.relocatable_module("/foo/baz", 0x10000000), module)
        self.assertNotEqual(prog.shared_library_module("/foo/bar", 0x10000000), module)
...
class TestLinuxUserspaceCoreDump(TestCase):
    def setUp(self):
        ...
        self.prog.set_enabled_module_debug_info_finders(["standard"])
    ...
    def test_loaded_modules(self):
        ...
        with self.subTest(module="main"):
            module = self.prog.find_main_module()
        ...
        with self.subTest(module="crashme"):
            module = self.prog.find_shared_library_module(
        ...
        with self.subTest(module="libc"):
            module = self.prog.find_shared_library_module(
                "/lib64/libc.so.6", 0x7FE154D0BB80
            )
            found_modules.append(module)
        ...
        with self.subTest(module="vdso"):
            module = self.prog.find_vdso_module("linux-vdso.so.1", 0x7FFEB18EC3E0)
            found_modules.append(module)
    ...
    def test_loaded_modules_pie(self):
        ...
        with self.subTest(module="libc"):
            module = self.prog.find_shared_library_module(
                "/lib64/libc.so.6", 0x7FB63CE22B80
            )
            found_modules.append(module)
        ...
        with self.subTest(module="vdso"):
            module = self.prog.find_vdso_module("linux-vdso.so.1", 0x7FFF5557C3E0)
            found_modules.append(module)
    ...
    def test_loaded_modules_static(self):
        ...
        with self.subTest(module="vdso"):
            module = self.prog.find_vdso_module("linux-vdso.so.1", 0x7FFC12B533E0)
            found_modules.append(module)
    ...
    def test_loaded_modules_static_pie(self):
        ...
        with self.subTest(module="vdso"):
            module = self.prog.find_vdso_module("linux-vdso.so.1", 0x7FFD67DF13E0)
            found_modules.append(module)
    ...
    def test_loaded_modules_pie_no_headers(self):
        ...
        with self.subTest(module="vdso"):
            module = self.prog.find_vdso_module("linux-vdso.so.1", 0x7FFCE47BB3E0)
            found_modules.append(module)
        ...
        with self.subTest(module="libc"):
            module = self.prog.find_shared_library_module(
                "/lib64/libc.so.6", 0x7F424B6E4B80
            )
            found_modules.append(module)
            self.assertIsNone(module.address_range)
            self.assertIsNone(module.build_id)
