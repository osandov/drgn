...
# TODO: maybe generalize to decimal_or_hexadecimal?
def pid_or_task(s: str) -> Tuple[str, int]:
    try:
        return "pid", int(s)
    except ValueError:
...
@crash_command(
    ...
    arguments=(
        argument(
            ...
            "Defaults to the initial mount namespace",
        ),
        drgn_argument,
    ),
)
def _crash_cmd_mount(
    ...
    if args.drgn:
        ...
        if args.task is None:
            ...
            if args.task[0] == "pid":
                print("task = find_task(pid)")
            else:
                print('task = Object(prog, "struct task_struct *", address)')
    ...
    if args.task is None:
        ns_or_prog: Union[Program, Object] = prog
    else:
        if args.task[0] == "pid":
            task = find_task(args.task[1])
        else:
            task = Object(prog, "struct task_struct *", args.task[1])
        ns_or_prog = task.nsproxy.mnt_ns
    ...
    for mnt in for_each_mount(ns_or_prog):
        rows.append(
            (
