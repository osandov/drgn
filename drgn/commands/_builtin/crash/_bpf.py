...
for bpf_map in bpf_map_for_each(prog):
    ...
    if args.prog_id:
        ...
        # The 'load_time' field was added in Linux kernel commit cb4d2b3f03d8e
        # in version 4.14-rc1. Older kernels may not have this field, so we catch
        # AttributeError and default to "(unknown)".
        try:
        ...
        # The 'name' field was added in Linux kernel commit 067cae47771c8
        # in version 4.14-rc3. Older kernels may not have this field, so we catch
        # AttributeError and default to "(unknown)".
        try:
            prog_name = (
                aux.name.string_().decode("utf-8", errors="replace").rstrip("\x00")
            )
            if not prog_name:
                prog_name = "(unused)"
        except AttributeError:
