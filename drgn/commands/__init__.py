...
from _drgn_util.typingutils import copy_func_params, copy_method_params
from drgn import Program, ProgramFlags, execscript
...
def _shell_command(source: str) -> Iterator[_ParsedShellCommand]:
    ...
    try:
        ...
        if len(exceptions) > 1 and sys.version_info >= (3, 11):
...
class CommandArgumentError(CommandError):
    """Error raised when a command's arguments could not be parsed."""
...
class CommandNamespace:
    ...
    def __init__(self, *, func_name_prefix: str = "_cmd_") -> None:
        """
        ...
        self.func_name_prefix = func_name_prefix
...
"""The default command namespace used by the drgn CLI."""
...
def _command_name(
    ...
    if name is None:
        ...
        match = re.fullmatch(namespace.func_name_prefix + r"(\w+)", func.__name__)
        if not match:
            raise ValueError(
                f"{func.__name__!r} doesn't start with {namespace.func_name_prefix}; "
...
def command(
    ...
    *arguments* using :mod:`argparse`. See :func:`custom_command()` for an
    ...
    def decorator(func: CommandFunc) -> CommandFunc:
        ...
        # TODO: document
        parser.register("type", "integer", lambda s: int(s, 0))
...
def linux_kernel_command(*args: Any, **kwargs: Any) -> CommandFuncDecorator:
    """
    Like :func:`command()`, but only enables the command when debugging the
    Linux kernel.
...
def linux_kernel_custom_command(
    *args: Any, **kwargs: Any
    ...
    """
    Like :func:`custom_command()`, but only enables the command when debugging
    the Linux kernel.
...
@custom_command(description="TODO", usage="let var = TODO", help="TODO")
def _cmd_let(
    prog: Program, name: str, args: str, *, globals: Dict[str, Any], **kwargs: Any
) -> None:
    print(repr(args))  # TODO
