...
from drgn.helpers.linux.kernfs import _kernfs_node_type, kernfs_children, kernfs_walk
...
def sysfs_listdir(prog: Program, path: str) -> List[bytes]:
    ...
    children = kernfs_children(kn)
    if not children:
        return []

    return [child.name.string_() for child in children]
