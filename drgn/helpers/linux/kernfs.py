...
def sysfs_lookup_node(prog: Program, path: str) -> Optional[Object]:
    ...
    :raises LookupError: If ``sysfs_root_kn`` symbol is not present
    """
    try:
        root_kn = prog["sysfs_root_kn"]
    except KeyError as e:
        raise LookupError("sysfs_root_kn symbol not found") from e
...
def sysfs_lookup(prog: Program, path: str) -> Optional[Object]:
    ...
    For class and bus objects, the container is reconstructed through
    ``struct subsys_private`` and the associated ``kset``. For driver
    objects, the container is obtained through ``struct driver_private``.
