...
__all__ = (
    ...
    "bpf_prog_by_id",
    "bpf_map_by_id",
    "_ringbuf_map_mem_usage",
    "_bpf_map_memory_size",
...
def bpf_map_memory_usage(bpf_map: Object) -> typing.Optional[int]:
    """
    Get the memory usage of a BPF map in bytes.

    This function handles version-specific logic to calculate the memory usage
    of a BPF map across different kernel versions. The memory accounting
    mechanism changed significantly across kernel versions:

    - **Linux 5.3 to 5.10**: Memory usage is tracked in ``struct bpf_map::memory.pages``.
      Commit 3539b96e041c ("bpf: group memory related fields in struct bpf_map_memory")
      (in v5.3) moved the ``pages`` field into a new ``struct bpf_map_memory``.
      This struct was removed by commit 80ee81e0403c ("bpf: Eliminate rlimit-based
      memory accounting infra for bpf maps") (in v5.11).

    - **Linux 4.10 to 5.2**: Memory usage is tracked in ``struct bpf_map::pages``
      directly on the map structure, added by commit aaac3ba95e4c
      ("bpf: charge user for creation of BPF maps and programs") (in v4.10).

    - **Linux 5.11+**: No explicit memory tracking field. The kernel uses
      an approximation based on key_size, value_size, and max_entries
      (see bpf_map_memory_footprint() in kernel/bpf/syscall.c).

    - **Linux 5.8+**: Special handling for RINGBUF maps, added by commit
      457f44363a88 ("bpf: Implement BPF ring buffer and verifier support for it")
      (in v5.8).

    :param bpf_map: ``struct bpf_map *`` object
    :return: Memory usage in bytes, or ``None`` if it cannot be determined
    """
    prog = bpf_map.prog_
    PAGE_SIZE = typing.cast(int, prog["PAGE_SIZE"].value_())
    ...
    try:
        ...
        return typing.cast(int, memory_pages * PAGE_SIZE)
    except AttributeError:
        pass
    ...
    try:
        ...
        return typing.cast(int, pages * PAGE_SIZE)
    except AttributeError:
        pass
    ...
    # This matches the kernel's bpf_map_memory_footprint() approximation
    # used in Linux 5.11+ (commit 80ee81e0403c) and is also used for
    # older kernels (pre-4.10) or map types where memory usage isn't tracked explicitly.
    # Formula: round_up(max_entries * round_up(key_size + value_size, 8), PAGE_SIZE)
    try:
        key_size = typing.cast(int, bpf_map.key_size.value_())
        value_size = typing.cast(int, bpf_map.value_size.value_())
        max_entries = typing.cast(int, bpf_map.max_entries.value_())
        calculated_size = _bpf_map_memory_size(
            bpf_map, key_size, value_size, max_entries
        )
...
def _ringbuf_map_mem_usage(prog: Program, bpf_map: Object, page_size: int) -> int:
    ...
    This function replicates the logic from kernel/bpf/ringbuf.c:ringbuf_map_mem_usage()

    :param prog: Program object
    :param bpf_map: ``struct bpf_map *`` object (should be of type BPF_MAP_TYPE_RINGBUF)
    :param page_size: System page size
    :return: Total memory usage in bytes, or 0 on error
    """

    try:
        bpf_ringbuf_map_type = prog.type("struct bpf_ringbuf_map")
        usage = typing.cast(int, bpf_ringbuf_map_type.size)
...
def _bpf_map_memory_size(
    bpf_map: Object, key_size: int, value_size: int, max_entries: int
) -> int:
    """
    Calculate BPF map memory size based on key, value sizes and entry count.

    :param bpf_map: ``struct bpf_map *`` object
    :param key_size: Size of map keys in bytes
    :param value_size: Size of map values in bytes
