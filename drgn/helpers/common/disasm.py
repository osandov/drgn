...
def _get_diasassembler(prog: Program) -> "capstone.Cs":
    # Get a disasembler for this specific program. This may be a cached instance
...
def _symbolize_dest(
    ...
    if i.id == 0 or prog.platform is None or len(i.operands) == 0:
        return None
    op = i.operands[-1]
    if op.type == capstone.CS_OP_IMM:
        is_64_bit = PlatformFlags.IS_64_BIT in prog.platform.flags
        addr_mask = 0xFFFFFFFFFFFFFFFF if is_64_bit else 0xFFFFFFFF
        op_addr = op.imm & addr_mask
...
_INSTRUCTION_ALIGNMENT: Dict[Architecture, int] = {
    Architecture.X86_64: 1,
    Architecture.I386: 1,
    Architecture.AARCH64: 4,
    Architecture.ARM: 2,
    Architecture.PPC64: 2,
    Architecture.RISCV32: 2,
...
def _find_mc_forward(prog: Program, start: int, count: int) -> Tuple[int, int]:
    if not prog.platform:
        raise ValueError("cannot disassemble without platform")
    alignment = _INSTRUCTION_ALIGNMENT[prog.platform.arch]
    disassembler = _get_diasassembler(prog)
    current_iaddr = start
    for _ in range(count):
        current_ilen = 0
        while True:
            current_ilen += alignment
            machine_code = prog.read(current_iaddr, current_ilen)
            try:
...
def _find_mc_reverse(prog: Program, start: int, count: int) -> Tuple[int, int]:
    ...
    # remaining instructions nibbling a platform-specific number of bytes
    alignment = _INSTRUCTION_ALIGNMENT[prog.platform.arch]
    disassembler = _get_diasassembler(prog)
    current_iaddr = start
    ...
    for _ in range(count):
        ...
        while True:
            current_ilen += alignment
            machine_code = prog.read(current_iaddr - current_ilen, current_ilen)
            try:
                (addr, size, mnem, _) = next(
