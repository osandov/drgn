...
int main(int argc, char **argv)
{
	...
	for (i = 0; i < 16; ++i)
		k[i] = i;
	key[0] = siphash_u8_to_le64(&k[0]);
	key[1] = siphash_u8_to_le64(&k[8]);
	...
	for (i = 0; i < 64; ++i) {
		...
		siphash_update(&hash, in, i);
		h = siphash_final(&hash);
		if (memcmp(&h, vectors_sip64[i], sizeof(h)) != 0)
			return EXIT_FAILURE;
	}

	for (i = 0; i < 64; ++i) {
		struct siphash128 hash;
		uint64_t h[2];

		siphash128_init(&hash, key);
		siphash128_update(&hash, in, i);
		siphash128_final(&hash, h);
		if (memcmp(&h, vectors_sip128[i], sizeof(h)) != 0)
			return EXIT_FAILURE;
	}
	return status;
}
